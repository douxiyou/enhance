...
func (s *Service) NewRequest4(dhcp *dhcpv4.DHCPv4) *Request4 {
	requestId := fmt.Sprintf("%s-%s", uuid.New().String(), dhcp.TransactionID.String())
	return &Request4{
		DHCPv4:    dhcp,
		Context:   context.WithValue(s.ctx, contextRequestID{}, requestId),
		peer:      &net.UDPAddr{},
		log:       s.log.With(zap.String("request", requestId)),
		requestId: requestId,
	}
}
